<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right">
            ...
            <a href="{{ route('committee_meeting.index') }}"
        ...
        <div class="fixed inset-0 z-0 pointer-events-none print:block hidden">
            <div class="w-full h-full flex flex-wrap content-center justify-center opacity-5 transform -rotate-45 text-gray-500 text-xs">
                @for ($i = 0; $i < 500; $i++)
                    <div class="p-0.5 whitespace-nowrap">Board Meeting {{ $committeeMeeting->me_id }} : <span style="font-size: 2px;"></span></div>
                @endfor
            </div>
        </div>
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden sm:rounded-lg pt-4 pb-2 ">
                        ...
                        <tr>
                            <td style="width: 33.33%">
                                <div style="float: left; margin-left: 10%">
                                    @php $test_report_data = $committeeMeeting->slug . ' Board Meeting '. "\n". $committeeMeeting->id . "\n" . \Carbon\Carbon::parse($committeeMeeting->date_and_time)->format('d-M-Y H:i:s') ; @endphp
                    ...
                    <table style="margin-top: 10px; font-size:18px; line-height: 1.2; text-align: center; font-weight: bold;">
                        ...
                        <tr>
                            <td>
                                ...
                                Date & Time: {{ \Carbon\Carbon::parse($committeeMeeting->date_and_time)->format('d-M-Y H:i:s') }} <sup>PST<span style="font-size: 3px;margin: 0px; padding: 0px;">{{\Illuminate\Support\Facades\Auth::user()->id}}</span></sup>
                                <br>
                                <span style="font-size: 14px;">MUID: {{ $committeeMeeting->id }}</span>
                                ...
                                @foreach($committeeMeeting->agendaItems->sortBy('order') as $ai)
                                    ...
                                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        ...
                                        <td class="py-1 px-2 text-center print:hidden">
                                            ...
                                            @can('committee_meeting_agenda_item-edit')
                                                <a href="{{ route('committee_meeting.agenda_item.edit', [$committeeMeeting->id, $ai->id]) }}"
                                                   class="inline-flex items-center px-4 py-2 bg-red-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                                    Edit
                                                </a>
                                            @endcan

                                            @can('committee_meeting_agenda_item-view')
                                                <a href="{{ route('committee_meeting.agenda_item.show', [$committeeMeeting->id, $ai->id]) }}"
                                                   class="inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                                    View
                                                </a>
                                            @endcan
                    ...
                    @can('committee_meeting.agenda_item.create')
                        <form method="POST" action="{{ route('committee_meeting.agenda_item.store', $committeeMeeting->id) }}" enctype="multipart/form-data" class="print:hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-4 mt-4 pl-8 pb-4 pt-4 pr-8">
                                ...
                                <input type="hidden" value="{{ $committeeMeeting->id }}" name="meeting_id">
